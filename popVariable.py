#solves inverse SIR problem with population as a variable (provides effective susceptible population estimate)
...
def data_sum(path):
    df = pd.read_csv(path)
    df = df[df['Country/Region']=='China'].transpose().iloc[4:, :]
...
def get_covid_data():
    ...
    sus = np.array([52e6-inf[i] for i in range(len(inf))])
...
def main():
    ...
    def odes(x, y):
        ...
        de1 = ds_x + (beta*i*s/(N*1e9))
        de2 = di_x - (beta*i*s/(N*1e9)) + (gamma*i)
        de3 = dr_x - (gamma*i)
        return [de1, de2, de3]
    ...
    ic1 = dde.IC(geom, lambda x: (1e9*N)-initial_infected-initial_removed * np.ones(x.shape), boundary, component=0)
    ...
    data = dde.data.PDE(geom, odes, [ic1, ic2, ic3, observe_y1, observe_y2], num_domain=900, num_boundary=20, anchors=observe_t)
    net = dde.maps.FNN([1] + [40] * 3 + [3], 'tanh', 'Glorot uniform')
    model = dde.Model(data, net)

    model.compile("adam", lr=.0001) # try 'log cosh'
    variable = dde.callbacks.VariableValue(
        [beta, gamma, N], period=1000, filename = "variables.dat"
    )
